  ...
  <div class="block" :class="blockClasses">
    <div class="block__header" @dblclick.prevent="toggleExpand()">
      ...
      <div class="block__actions" v-if="withActions">
        ...
        <a17-button
          variant="icon"
          @click="toggleFavoriteForm = !toggleFavoriteForm"
          :style="block?.favorite ? 'background: #3278B8;' : ''"
          ><span v-svg symbol="star-feature"></span
        ></a17-button>
        <input
          id="favoriteFormName"
          v-if="toggleFavoriteForm"
          placeholder="Name"
          v-model="favoriteBlockName"
          :disabled="block?.favorite"
          style="vertical-align: top"
        />
        <a17-button
          v-if="toggleFavoriteForm"
          variant="icon"
          @click="postToggleFavoriteForm"
          ><span v-svg :symbol="block?.favorite ? 'trash' : 'check'"></span
        ></a17-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        toggleFavoriteForm: false,
        favoriteBlockName: this.block?.favorite?.title ?? ''
      }
    },
    ...
    computed: {
      ...
      ...mapState({
        ...
        toggleIsFavoriteUrl: state => state.form.toggleIsFavoriteUrl
      }),
      ...mapGetters([
        'fieldValueByName'
      ])
    ...
    methods: {
      ...
      async postToggleFavoriteForm() {
        await this.$store.dispatch(ACTIONS.TOGGLE_BLOCK_IS_FAVORITE, {
          block: this.block,
          favoriteBlockName: this.favoriteBlockName
        })
        this.toggleFavoriteForm = false
      },
