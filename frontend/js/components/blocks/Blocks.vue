            ...
            >
              ...
              >
                ...
                <div slot="dropdown-action">
                  ...
                  <button type="button" v-else @click="expandAllBlocks()">
                  ...
                  >
                    {{
                      $trans(
                        'fields.block-editor.open-in-editor',
                        'Open in editor'
                      )
                    }}
                  </button>
                  ...
                  <button
                    type="button"
                    @click="handleCopyToClipboard(copyBlock, block)"
                  >
                    {{ $trans('fields.block-editor.copy-block', 'Copy block') }}
                  </button>
                  <button
                    v-if="$store.state.blocks.blockClipboard"
                    type="button"
                    @click="handlePaste(pasteBlock, blockIndex, block)"
                  >
                    {{
                  ...
                  >
                    {{
                      $trans(
                        'fields.block-editor.create-another',
                        'Create another'
                      )
                    }}
                  </button>
                  <button type="button" @click="handleDeleteBlock(remove)">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters(['blocks', 'fieldsByBlockId', 'availableBlocks'])
    },
    methods: {
      setOpened: function() {
        const allClosed =
      ...
      handleOnEnd(moveFn, moveBlockToEditorFn) {
        ...
        const { block, editorName, newIndex, index } = this.nextMove

        if (block.name !== editorName) {
          if (this.checkIfBlockTypeIsAvailable(editorName, block.type)) {
            moveBlockToEditorFn &&
              moveBlockToEditorFn(block, editorName, index, newIndex)
          }
        } else {
      ...
      handleCopyToClipboard(copyFn, block) {
        copyFn && copyFn({ block })
      },
      handlePaste(pasteFn, blockIndex, block) {
        pasteFn && pasteFn({ block, index: blockIndex + 1 })
        this.$nextTick(() => {
          this.checkExpandBlocks()
        })
      },
      ...
      checkIfBlockTypeIsAvailable(editorName, type) {
        const availableBlocks = JSON.parse(
          JSON.stringify(this.availableBlocks(editorName))
        )
    ...
    mounted() {
      ...
      this.$nextTick(function() {
        if (
          this.$refs.blockList &&
          this.blocks(this.editorName) &&
          this.blocks(this.editorName).length < 4
        ) {
